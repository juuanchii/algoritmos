...
class Graph:
    ...
    def show_graph(self):
        ...
        for index, nodo in enumerate(self.elements):
            print(nodo['value'])
            if nodo['other_value'] is not None:
    ...
    def deep_show1(self, origin):
        def __deep_show1(graph, origin):
            pos_vertice = graph.search(origin)
            if pos_vertice is not None:
                if not graph.elements[pos_vertice]['visitado']:
                    ...
                    adyacentes = graph.elements[pos_vertice]['aristas']
                    for adyacente in adyacentes:
                        __deep_show1(graph, adyacente['value'])
        
        self.mark_as_not_visited()
        __deep_show1(self, origin)
    ...
    def amplitude_show1(self, origin):
        self.mark_as_not_visited()
        cola = Queue()
        paises = []
        tipo = []
        ...
        if pos_vertice is not None:
            if not self.elements[pos_vertice]['visitado']:
                ...
                while cola.size() > 0:
                    nodo = cola.attention()
                    if nodo['other_value'][2] not in paises:
                        paises.append(nodo)
                    nodo['visitado'] = True
                    print(nodo['value'])
                    adyacentes = nodo['aristas']
    ...
    def paises_con_maravillas(self):
        ...
        arq_nat =[]
        for index, nodo in enumerate(self.elements):
            if nodo['other_value'][1] not in paises:
                paises.append(nodo['other_value'][1])
            elif nodo['other_value'][2] not in arquitectonicos:
                arquitectonicos.append(nodo['other_value'][2])
            else:
                arq_nat.append(nodo['other_value'][1])
        return arq_nat