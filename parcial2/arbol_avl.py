...
class BinaryTree:
    ...
    def contar_tipo(self, key):
        def __contar_tipo(root):
            counter = 0
            if root is not None:
                if root.value == key:
                    counter = 1
                counter += __contar_tipo(root.left)
                counter += __contar_tipo(root.right)
            return counter

        return __contar_tipo(self.root)
    ...
    def inorden_data_pokemons(self):
        def __inorden_data_pokemons(root):
            if root is not None:
                __inorden_data_pokemons(root.left)
                print("--------------------------------")
                print(f"        Numero: {root.other_value['numero']}")
                print(f"        Nombre: {root.other_value["nombre"]}")
                print(f"        Tipo: {root.other_value['tipo']}")
                __inorden_data_pokemons(root.right)

        if self.root is not None:
            __inorden_data_pokemons(self.root)
    ...
    def proximity_search_pokemons(self, search_value):
        def __proximity_search_pokemons(root, search_value):
            if root is not None:
                __proximity_search_pokemons(root.left, search_value)
                if root.value.startswith(search_value):
                    print(f"Nombre pokemon {root.value}, numero: {root.other_value["numero"]}")
                __proximity_search_pokemons(root.right, search_value)

        if self.root is not None:
            __proximity_search_pokemons(self.root, search_value)
    ...
    def inorden_por_tipo(self, key):
        def __inorden_por_tipo(root):
            if root is not None:
                __inorden_por_tipo(root.left)
                if key in root.other_value["tipo"]:
                    print(f"Nombre= {root.other_value["nombre"]}, Tipo= {root.other_value["tipo"]}")
                __inorden_por_tipo(root.right)
